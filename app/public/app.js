...
class StudentRadioApp {
  ...
  async refreshAdminSession() {
    const { ok, data } = await this.apiGet('/auth/me');
    this.adminUser = ok ? data.admin : null;
    if (this.adminUser && this.adminUser.role === 'SUPER_ADMIN') {
  ...
  async submitAdminLogin() {
    ...
    const { ok, data } = await this.apiSend('/auth/login', 'POST', { email, password: pass });
    if (ok) {
      this.adminUser = data.admin;
      if (this.adminUser.role === 'SUPER_ADMIN') await this.refreshEditors();
      this.closeAdminLoginModal();
      this.updateAdminAuthUI();
      this.switchTab('admin');
      this.showToast(`Zalogowano pomyślnie jako ${this.adminUser.fullName} (${this.roleLabel(this.adminUser.role)})`, 'success');
    } else {
      this.showToast(data.message || 'Nieprawidłowe dane logowania.', 'warning');
    }
  }

  async logoutAdmin() {
    await this.apiSend('/auth/logout', 'POST');
    this.adminUser = null;
    this.editors = [];
    this.updateAdminAuthUI();
